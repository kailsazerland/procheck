...
class CashFlowController extends AppController {
    ...
    public function cash_in() {
        ...
        $this->paginate['CashFlow'] =  array('limit' => 30
                                            ,'conditions' => array_merge(array('CashFlow.type_id' => $this->CashFlow->Type->get_id_in()),$this->search_conditions)
                                            ,'order' => array('created' => 'DESC'));   
//var_dump($this->paginate['CashFlow']);die(); 
    ...
    public function cash_out() {
        ...
        $this->paginate['CashFlow'] =  array('limit' => 30
                                            ,'conditions' => array_merge(array('CashFlow.type_id' => $this->CashFlow->Type->get_id_out()),$this->search_conditions)
                                            ,'order' => array('created' => 'DESC'));   
...
//var_dump($filter_box_data['created']);die();        
        $conditions = array(
            ...
            if($filter_box_data['org_id'] != 0)
                $conditions['AND']['CashFlow.org_id'] = $filter_box_data['org_id'];
            if($filter_box_data['contragent_id'] != 0)
                $conditions['AND']['CashFlow.contragent_id'] = $filter_box_data['contragent_id'];
            if($filter_box_data['pay_type_id'] != 0)
                $conditions['AND']['CashFlow.pay_type_id'] = $filter_box_data['pay_type_id'];
            if($filter_box_data['article_id'] != 0)
                $conditions['AND']['CashFlow.article_id'] = $filter_box_data['article_id'];
            if($filter_box_data['nomenclature_id'] != 0)
                $conditions['AND']['CashFlow.nomenclature_id'] = $filter_box_data['nomenclature_id'];
            if($filter_box_data['classificator_id'] != 0)
                $conditions['AND']['CashFlow.classificator_id'] = $filter_box_data['classificator_id'];
            if($filter_box_data['autor'] != 0)
                $conditions['AND']['CashFlow.user_id'] = $filter_box_data['autor'];
    ...
    public function set_filter_option($type) {
        return array(
            'Orgs' => $this->CashFlow->Org->find('list',array('conditions' => array('orgs_group_id' => 99))),
            'Contragents' => $this->CashFlow->Contragent->get_list(array('conditions' => array('orgs_group_id' => $this->CashFlow->Contragent->OrgsGroup->find('list',array('fields' => array('id','id'),'conditions' => array('type_id != 99')))))),
            'PayTypes' => $this->CashFlow->PayType->find('list'),
            'Articles' => $this->CashFlow->Article->get_list(array('conditions' => array('type_id' => $type))),
            ...
            'Classificators' => $this->CashFlow->Classificator->find('list',array('conditions' => array('type_id' => $type))),
            'Autors' => $this->CashFlow->User->find('list'),
            ...
            'created_b' => (isset($this->filter_box_data['created_b']))?$this->filter_box_data['created_b']:'01.01.2015',
