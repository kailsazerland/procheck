...
<table class='table table-hover table-striped table-bordered table-condensed'>
    <thead>
        <tr>
            ...
            <th><?php echo $this->Admin->PaginatorSort('percent','Процент') ?></th>
            <th>Управление</th>
            
        </tr>
    </thead>
    <?php
        foreach ($nalogs AS $value)
        {
            echo $this->Html->TableCells(array(
                 $value['Nalog']['name']
                ,$value['Nalog']['percent']
                ,array('<div class="btn-group">' . $this->ActionPanel->DrawPanel($action_menu,$value['Nalog']['id']) . '</div>',array('class'=>''))
            ));
        }
    ?>    
</table>
